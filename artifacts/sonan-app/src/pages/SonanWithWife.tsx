...
export default function SonanWithWife() {
  ...
  useEffect(() => {
    setLoading(true);
    const file = language === "ar" ? "wife-sonan-data.json" : `wife-sonan-data.${language}.json`;

    fetch(`${import.meta.env.BASE_URL}data/${file}`)
      .then((r) => r.ok ? r.json() : fetch(`${import.meta.env.BASE_URL}data/wife-sonan-data.json`).then((r2) => r2.json()))
      .then((json: TextCollection) => { setData(json); setLoading(false); })
      .catch(() => setLoading(false));
  }, [language]);

  return (
    <PageLayout title={t("wife")} subtitle={data?.subtitle ?? t("wifeSubtitle")} backHref="/">
      <div className="pt-4 pb-10">
        {!loading && data && (
          <div className="mb-4 flex items-center justify-end">
            ...
            >
              {isRtl ? `${data.items.length} سنة` : `${data.items.length} items`}
            </span>
          </div>
        )}
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="space-y-2">
            {data.items.map((item, idx) => (
              <motion.div key={item.id} variants={rowAnim}>
                <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate(`/wife/${item.id}`)} className="w-full">
                  <div
